...
describe('server/controllers/SheetController', () => {
	describe('getSheet', () => {
		test('sheet exists | secret campaign + sheet | non-participant', async done => {
		...
		test('sheet exists | secret campaign + sheet | non-owner participant', async done => {
			mockingoose(Sheet).toReturn(SHEET({ secret: true }), 'findOne')
		...
		test('sheet exists | public campaign + sheet | non-participant', async done => {
		...
		test('sheet exists | secret campaign, public sheet | participant', async done => {
			mockingoose(Sheet).toReturn(SHEET({ secret: false }), 'findOne')
			const result = await getSheet('test', (
				CAMPAIGN({ secret: true }, { isParticipant: true })
			), ANY)
		...
		test('sheet exists | secret campaign + sheet | sheet owner', async done => {
			mockingoose(Sheet).toReturn(SHEET({ secret: true }), 'findOne')
			const result = await getSheet('test', (
				CAMPAIGN({ secret: true }, { isParticipant: true })
			), OWNER)
		...
		test('sheet exists | secret campaign + sheet | campaign editor', async done => {
			mockingoose(Sheet).toReturn(SHEET({ secret: true }), 'findOne')
			const result = await getSheet('test', (
		...
		test('sheet exists | secret campaign + sheet | campaign owner', async done => {
			mockingoose(Sheet).toReturn(SHEET({ secret: true }), 'findOne')
			const result = await getSheet('test', (
	...
	describe('permissions', () => {
		test('passes for Admin users', async done => {
		...
		test('returns 401 to non-viewers if required', async done => {
			mockingoose(Sheet).toReturn(SHEET(), 'findOne')
		...
		test('returns 401 to non-editors', async done => {
			const response = mockResponse()
			const next = jest.fn()
		...
		test('passes for non-owners if sheet is public', async done => {
			mockingoose(Sheet).toReturn(SHEET({ secret: false }), 'findOne')
			const response = mockResponse()
			const next = jest.fn()
	...
	describe('getSheetRequest', () => {
		test('returns 404 if sheet is not found', async done => {
			const response = mockResponse()
		...
		test('returns article if found', async done => {
			const sheet = SHEET()
			const response = mockResponse()
	...
	describe('upsertSheet', () => {
		test('accepts updates', async done => {
	...
	describe('deleteSheet', () => {
		test('performs delete if article is found', async done => {
